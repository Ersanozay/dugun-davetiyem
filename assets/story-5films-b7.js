const scenes = [
  {
    ...
    hevcVideo: '/assets/story-01-library-hevc.mp4',
  ...
  },
  {
    ...
    hevcVideo: '/assets/story-02-cards-hevc.mp4',
  ...
  },
  {
    ...
    hevcVideo: '/assets/story-03-graduation-hevc.mp4',
  ...
  },
  {
    ...
    html: '<span>Bir bağ bahçesinde geldi</span><br class="story-mobile-break"> <span>teklif, ardından da... Evet</span>',
    video: '/assets/story-04-proposal.mp4',
    hevcVideo: '/assets/story-04-proposal-hevc.mp4',
  ...
  },
  {
    ...
    hevcVideo: '/assets/story-05-final-hevc.mp4',
        ...
        <div class="video-story-layout">
          <div class="video-story-media">
            <video
              ...
              tabindex="-1"
            >
              <source src="${scene.hevcVideo}" type='video/mp4; codecs="hvc1"'>
              <source src="${scene.video}" type="video/mp4">
            </video>
            ${scene.finalFrame ? `<img class="video-story-final-frame" data-story-final-frame src="${scene.finalFrame}" alt="" aria-hidden="true" decoding="async">` : ''}
  ...
  return `
    <article class="desktop-story-card desktop-story-card--${scene.tone}" data-desktop-story-card data-story-index="${index}" aria-labelledby="desktop-story-title-${scene.id}" tabindex="0">
      <div class="desktop-story-card__media">
        ...
        ${scene.finalFrame ? `<img class="desktop-story-card__final-frame" data-story-final-frame src="${scene.finalFrame}" alt="" aria-hidden="true" decoding="async">` : ''}
...
function mountMobile(root, reducedMotion) {
  ...
  const update = () => {
    ...
    verticalSceneEls.forEach((sceneEl, verticalIndex) => {
      ...
      const finalFrame = sceneEl.querySelector('[data-story-final-frame]');

      const videoProgress = cfg.finalFrameAt ? Math.min(progress, cfg.finalFrameAt) : progress;
      setVideoTime(video, videoProgress, reduce);
      if (finalFrame) {
        const finalReveal = reduce ? 1 : smoothstep(cfg.finalFrameAt - 0.025, cfg.finalFrameAt + 0.035, progress);
        finalFrame.style.opacity = finalReveal.toFixed(3);
      }
...
function mountDesktop(root, reducedMotion) {
  ...
  const update = () => {
    ...
    const finalFrame = card?.querySelector('[data-story-final-frame]');

    if (video && Number.isFinite(video.duration) && video.duration > 0) {
      const rawMediaProgress = reduce ? 0.78 : local;
      const mediaProgress = cfg.finalFrameAt ? Math.min(rawMediaProgress, cfg.finalFrameAt) : rawMediaProgress;
      const targetTime = Math.min(Math.max(0, video.duration - 0.045), mediaProgress * Math.max(0, video.duration - 0.045));
      if (Math.abs(video.currentTime - targetTime) > 0.035) {
        try { video.currentTime = targetTime; } catch (_) {}
      }
    ...
    if (finalFrame) {
      const finalReveal = reduce ? 1 : smoothstep(cfg.finalFrameAt - 0.025, cfg.finalFrameAt + 0.035, local);
      finalFrame.style.opacity = finalReveal.toFixed(3);
    }
